...
Cleanup attempts every named resource, independently verifies that the container and both networks are absent, and persists the result in the ledger. A cleanup verification failure overrides an otherwise successful Probe result. Before dispatch, the reservation is rolled back only after verified cleanup; failed cleanup preserves a zero-attempt `PRE_DISPATCH_CLEANUP_FAILED` ledger.
