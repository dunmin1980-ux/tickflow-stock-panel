...
canary_receipt="${TICKFLOW_CANARY_BUILD_RECEIPT:-$root/reports/macos_intel_acceptance/security_canary_build.sha256}"
...
receipt_digest="$(tr -d '\r\n' < "$canary_receipt")"
[[ "$receipt_digest" =~ ^[0-9a-f]{64}$ ]] || fail "dedicated canary build receipt is invalid"
actual_digest="$(printf '%s' "$canary" | shasum -a 256 | awk '{print $1}')"
[[ "$receipt_digest" == "$actual_digest" ]] || fail "artifacts were not built with this canary"
